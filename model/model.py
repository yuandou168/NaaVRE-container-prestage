...
def load_data(data_path, input_shape=(224,224)):
    ...
    """
    ### START CODE HERE ###
    trainset = CustomDataset(partition_file="train_client_2", folder=data_path, input_shape=input_shape)
    valset = CustomDataset(partition_file="val_client_2", folder=data_path, input_shape=input_shape)
...
def train(net, trainloader, valloader, epochs):
    ...
    print("Time to train the whole network: ", end-start, " s")
        ...
        # Load model and data
net = Model().to(DEVICE)
trainloader, valloader, testloader, num_examples = load_data("../data/MIL_data/UvA_ROI")
...
fl.client.start_numpy_client("152.228.166.247:8080", client=HistologyClient())
